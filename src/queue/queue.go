...
func Distributor(localID string, assignedOrder <-chan types.Order, localOrder chan<- types.Button) {
	...
	for {
		select {
		case a := <-assignedOrder: //Send assignedOrder to network. If the order is yours, take the order.
			if a.Button != 2 {
		...
		case a := <-netRecv: //Recieving assignedOrder from network. Ignore if it's not your ID, only turn on lights.
			elevio.SetButtonLamp(a.Button, a.Floor, true)
...
//Takes a button press from hardware, calculates cost and sends an assignedOrder to Distributor.
...
//When a node leaves the network, LostPeers finds this elevator's hall orders and sends them to Distributor to get re-distributed.

func LostPeers(peerUpdateCh <-chan peers.PeerUpdate, allStatesToLostPeers <-chan map[string]types.ElevState, newOrder chan<- types.Button) {

	var states map[string]types.ElevState
	for {
		select {
		case states = <-allStatesToLostPeers:
		case a := <-peerUpdateCh:
			lost_id := a.Lost
			if len(lost_id) != 0 {
